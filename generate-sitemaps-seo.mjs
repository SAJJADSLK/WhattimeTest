...
async function generateAllSitemaps() {
  try {
    ...
    const citiesUrls = allCities.map((city) => ({
      loc: `${BASE_URL}/city-detail/${city.name.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/[\s\W]+/g, '-')}`,
      lastmod: today,
      changefreq: 'weekly',
      priority: '0.7',
    }));
    ...
    const countriesUrls = allCountries.map((country) => ({
      loc: `${BASE_URL}/country/${country.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/[\s\W]+/g, '-')}`,
      lastmod: today,
      changefreq: 'weekly',
      priority: '0.8',
    }));
