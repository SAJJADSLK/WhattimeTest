...
import { getSunTimes, getNextDstChange } from '../time.js';
import { getCountryInfo } from '../countryData.js';
...
getAll: publicProcedure
  ...
  /**
   * Get enhanced city data with DST, phone codes, and sun times
   */
  getEnhanced: publicProcedure
    ...
    .query(async ({ input }) => {
      try {
        ...
        const city = allCities.find(c => c.id === input.cityId);
        if (!city) return null;
        ...
        // Get next DST change
        const nextDstChange = getNextDstChange(city.timezone);

        // Get country info (phone code, DST rules)
        const countryInfo = getCountryInfo(city.country);

        return {
          sunTimes,
          nextDstChange,
          phoneCode: countryInfo?.phoneCode || 'N/A',
          dstObserved: countryInfo?.dstObserved || false,
          dstStart: countryInfo?.dstStart,
          dstEnd: countryInfo?.dstEnd,
        };
      } catch (e) {
        console.error("Error in getEnhanced:", e);
        return null;
      }
    }),
});
